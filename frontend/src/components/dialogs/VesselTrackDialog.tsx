...
import {
  ...
  Button, CircularProgress, Slider, IconButton, Grid, Divider,
} from "@mui/material";
import { Close, PlayArrow, Pause, Replay, Route } from "@mui/icons-material";
...
export default function VesselTrackDialog({ vessel, open, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    stopPlay();
    const n = new Date();
    setDateFrom(toLocalInput(new Date(n.getTime() - 24 * 3600 * 1000)));
    setDateTo(toLocalInput(n));
  }, [vessel?.mmsi, open, stopPlay]);
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="lg" fullWidth>
      ...
      <DialogContent sx={{ backgroundColor: THEME_COLORS.background, p: 0 }}>
        ...
        {current && (
          <Box sx={{ px: 2.5, py: 1.5, backgroundColor: THEME_COLORS.surface }}>
